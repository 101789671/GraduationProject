...
<div class="panel admin-panel">
		<div class="panel-head" id="add">
			<strong><span class="icon-pencil-square-o"></span>学生进度</strong>
			</div>
<div class="padding border-bottom">
			<ul class="search" style="padding-left:10px;">
				<form method="post" action="<%=path%>/tut/Tutor_studentProgress.action?">
					<li>查询：<select class="select2" name="student.stuNo" id="select2"
						>
					</select> 
					</li>
	...
	<div id="progressSS" class="gpProgresDivSS">
		...
		<s:iterator id="item" value="studentList" status="i">
		 <div class="studentProgressMiddleText"><div style="font-weight: bolder;float: left;">学生姓名:</div>${stuName}</div>
		 <div class="studentProgressMiddleText"><div style="font-weight: bolder;float: left;margin-left: 30px;">班级:</div>${className}</div>
		 </s:iterator>
		</div>
	</div>
	</div>
 ...
 */
	...
	myProgres[9] = "${item.stuName}";
	myProgres[10] = "${item.className}";
	</c:forEach>
	...
	proress[0] = "${stuNo}";
	proress[1] = "${stuName}";
	studentlist.push(proress);
	</s:iterator>
	
	
	for(i=0;i<studentlist.length;i++){
	var select2 = document.getElementById("select2");
         var addOption=new Option(""+studentlist[i][1]+"",""+studentlist[i][0]+"");//生成一个选项
         select2.options.add(addOption); //这个兼容IE与firefox
         }
	porgressTop();
	progressBottom();
</script>
